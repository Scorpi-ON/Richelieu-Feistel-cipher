...
import { bitsToNumber, blockToThreads, numberToBits, threadsToBlock } from "../../ts/feistel/bitwise_operations";
import { Bit, BLOCK_SIZE, THREAD_COUNT, THREAD_SIZE } from "../../ts/feistel/constants";
...
describe("Функции операций с блоками, потоками и битами", () => {
    describe("numberToBits", () => {
        ...
        test.each([
            [-8, 10],
            [-10, 10],
        ])("Если переводимое число отрицательное, то возникает ошибка", (number: number, bitCount: number) => {
    ...
    describe("blockToThreads", () => {
        test.each([
            [(1n << BigInt(BLOCK_SIZE)) - 1n, Array(THREAD_COUNT).fill((1n << BigInt(THREAD_SIZE)) - 1n)],
            [
                BigInt(
                    "0b" +
        ...
        ])("Если передан блок, то возвращает его потоки", (block: bigint, expected: readonly bigint[]) => {
    ...
    describe("threadsToBlock", () => {
        test.each([
            [Array(THREAD_COUNT).fill((1n << BigInt(THREAD_SIZE)) - 1n), (1n << BigInt(BLOCK_SIZE)) - 1n],
            [
                [
                    0n,
                    BigInt(0b10100101111000010110000101110101),
        ...
        ])("Если переданы потоки, то возвращает блок из них", (threads: readonly bigint[], expected: bigint) => {
