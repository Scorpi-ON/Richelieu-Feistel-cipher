...
const COLOR_SCHEME_MEDIA: MediaQueryList = window.matchMedia("(prefers-color-scheme: dark)");
...
export default function setupTheming(): void {
    ...
    const switchToColorScheme = (scheme: ColorScheme): void => {
        themeSwitchButton.innerText = scheme === "dark" ? COLOR_SCHEME_CAPTIONS.light : COLOR_SCHEME_CAPTIONS.dark;
    ...
    themeSwitchButton.onclick = (): void => {
        const newColorScheme: ColorScheme = document.body.getAttribute("data-theme") === "dark" ? "light" : "dark";
        switchToColorScheme(newColorScheme);
    };
}
